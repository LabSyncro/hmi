...
const removeDeviceItem = (
  device: Device & { items: TransportDeviceItem[] },
  itemId: string
) => {
  device.items = device.items.filter(
    (item) => item.id !== itemId
  ) as TransportDeviceItem[];
...
const completeTransport = async () => {
  ...
  try {
    toast({
      title: "Thành công",
      description: "Đã hoàn tất ghi nhận vận chuyển thiết bị",
    });
    resetForm();
  } catch (error) {
    toast({
    ...
    <div class="grid grid-cols-3 gap-6">
      ...
      >
        ...
        <div>
          ...
          <div
            v-if="mode === 'transport' && devices.length > 0"
          ...
          >
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="grid grid-cols-2 w-full">
                ...
                <p class="font-medium text-gray-800">
                  {{ transportDetails.transportDate }}
                </p>
              </div>
            </div>
